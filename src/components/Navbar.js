import React from 'react'

const Navbar = () => {
    return (
        <div className='container'>
            <nav className='navbar'>
                <div className='nav-container'>
                    <div className='nav-box'>
                        <div className='main-logo'>
                            logo
                        </div>
                        <div className="menu">
                            <a href="/#about" className='menu-link'>About</a>
                            <a href="/#aggregator" className='menu-link'>Aggregator</a>
                            <a href="/#features" className='menu-link'>Features</a>
                            <a href="/#roadmap" className='menu-link'>Roadmap</a>
                            <a href="/#contact" className='menu-link'>Contact</a>
                        </div>
                        <div className='side-link'>
                            <div className='social-link'>
                                <a href="/#"></a>
                                <a href="">logo</a>
                            </div>
                        </div>
                    </div>

                </div>
            </nav>
            <div className='homepage'>
                <div className='home-container'>
                    <div className='home-box'>
                        <div className='home-content'>
                            <div className='home-div'>
                                <img src="" alt="img" />
                                <div className='home-intro'>
                                    <span className='home-head1'>NFD platform</span>
                                    <span className='home-head2'>"NFD.gg"</span>
                                </div>
                                <div className='home-inro-text'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Officia provident totam sed corrupti aliquid ullam aut odit esse earum vero!
                                </div>
                                <div className='home-feature'>
                                    <div className='home-feature-box'>
                                        <div className='home-feature-img'>
                                            <img src="" alt="img" />
                                        </div>
                                        <div className='home-feature-text'>Info in one place</div>
                                    </div>
                                    <div className='home-feature-box'>
                                        <div className='home-feature-img'>
                                            <img src="" alt="img" />
                                        </div>
                                        <div className='home-feature-text'>Private tools</div>
                                    </div>
                                    <div className='home-feature-box'>
                                        <div className='home-feature-img'>
                                            <img src="" alt="img" />
                                        </div>
                                        <div className='home-feature-text'>Wide Functionality</div>
                                    </div>
                                </div>
                                <div className='home-feature-btn'>
                                    <div className='home-btn-name'>
                                        <a href="#">
                                            <button className='btn'>
                                                <div className='btn-design'></div>
                                                <span></span>
                                                <div className='home-btn-text'>Try Platform</div>
                                                <div className='btn-design'></div>
                                            </button>
                                        </a>
                                    </div>
                                    {/* <div className='home-btn2'>
                                        <button>
                                            <div className='btn-design2'></div>
                                            <span></span>
                                            <div className='home-btn-text'>Only PC version</div>
                                            <div className='btn-design2'></div>
                                        </button>
                                    </div> */}
                                    <button className='home-btn'>How it works?</button>
                                </div>
                                <div className='grad-design'></div>
                            </div>
                            <div className='home-second-box'>
                                <img src="" alt="img" />
                                <div className='home-sec-textbox'>
                                    <span className='home-sec-head'>NFD software</span>
                                    <span className='home-sec-word'>"Combine"</span>
                                </div>
                                <div className='home-sec-text'>Lorem ipsum dolor sit amet consectetur adipisicing elit.</div>
                                <div className='home-sec-btnbox'>
                                    <button className='home-sec-btn'>
                                        <div className='home-sec-btn-contain'>
                                            <img src="" alt="" />
                                            <span className='home-sec-btn-text'>macOs</span>
                                            </div>
                                        <div className='home-sec-btn-design'>
                                            <img src="" alt="" />
                                            </div>
                                    </button>
                                    <button className='home-sec-window-btn'>
                                        <div className='home-window-btn'>
                                            <img src="" alt="" />
                                            <span className='home-sec-btn-text'>
                                                Windows</span>
                                                </div>
                                        <div className='home-window-design'><img src="" alt="" /></div>
                                    </button>
                                </div>
                                <div className='home-btn-sec'>
                                    <div className='home-btn-sec-box'>
                                        <a href="#">
                                            <button className='btn'>
                                                <div className='btn-design'></div>
                                                <span></span>
                                                <div className='home-btn-text'>Download</div>
                                                <div className='btn-design'></div>
                                            </button>
                                        </a>
                                    </div>
                                    {/* <div>
                                        <button>
                                            <div></div>
                                            <span></span>
                                            <div>Only PC version</div>
                                            <div></div>
                                        </button>
                                    </div> */}
                                    <a href="#">
                                        <button className='home-btn'>How it works?</button>
                                    </a>
                                </div>
                                <div></div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            {/* about page */}
            <div className='about'>
                <div className='about-page'>
                    <div className='about-box'>
                        <div className='about-box-content'>
                            <h2>The first web3 crypto platform</h2>
                            <h3>Problem</h3>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ullam molestias quasi ducimus! Et ea dolores aliquam minima totam, quia sit.</p>
                        </div>
                        <div>
                            <video src=""></video>
                            <img src="https://www.nfd.gg/_next/image?url=%2Fblock1OPT.webp&w=750&q=75" alt="" />
                        </div>
                    </div>
                    <div className='about-desc'>
                        <div className='about-desc-box'>
                            <h2>We work to make your life easier</h2>
                            <h3>Solution</h3>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempora eum blanditiis dolorem recusandae, consequatur pariatur accusantium. Provident labore aliquid harum! Dolor beatae dolorem asperiores cumque exercitationem expedita cum sunt harum.</p>
                            <div className='about-head'>
                                <hr />
                                <div>
                                    <div>
                                        <img src="" alt="" />
                                    </div>
                                    <span>NFD will save you time and help you earn money in crypto.</span>
                                </div>
                            </div>
                        </div>
                        <div></div>
                    </div>
                    <div className='about-explain'>
                        <div className='about-explain-box'>
                            <h1>The first web3 crypto platform</h1>
                            <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. Fugiat, quas.</span>
                        </div>
                        <div className='about-box'>
                            <div className='about-box-box'>
                                <div className='about-box-box-box'>
                                    <div className='about-box4'>
                                        "0"
                                        "1"
                                    </div>
                                    <div className='about-box5'>
                                        <h3>Problem</h3>
                                        <h4>Crypto infonoise</h4>
                                    </div>
                                </div>
                                <img src="" alt="" />
                            </div>
                            <div className='about-photo'>
                                <img src="" alt="" />
                                <div className='about-photo-head'>The first web3 crypto platform</div>
                                <span className='about-photo-text'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Odit rem ipsam iste similique! Officia consectetur quibusdam, sequi eveniet eos provident doloremque expedita temporibus eius labore, non maiores explicabo nisi nulla.</span>
                            </div>
                        </div>
                        <div className='about-item'>
                            <div className='about-item-box'>
                                <div className='about-item-name'>
                                    <div className='about-item-design'>
                                        "0"
                                        "2"
                                    </div>
                                    <div className='about-item-text'>
                                        <h3>Solution</h3>
                                        <h4>NFD platform</h4>
                                    </div>
                                </div>
                                <img src="" alt="" />
                            </div>
                        </div>
                    </div>
                    {/* <div className='aboutbox'>
                        <div></div>
                        <img src="" alt="" />
                        <img src="" alt="" />
                    </div> */}
                </div>
            </div>

            {/* aggregator page */}
            <div className='aggerator'>
                <div className='aggerator2'></div>
                <div className='aggerator3'></div>
                <div className='aggerator-imgbox'>
                    <img src="" alt="" />
                </div>
                <div className='aggerator-imgbox2'>
                    <img src="" alt="" />
                </div>
                <div className='aggerator-imgbox3'>
                    <img src="" alt="" />
                </div>
                <div className='box1'>
                    {/* <div className='box2'>
                        <div className='box3'>
                            "No spam, only"
                            <div className='box-head'>Alpha</div>
                        </div>
                        <div className='box-text'>
                            Lorem ipsum dolor sit amet consectetur adipisicing elit. Optio, earum!
                        </div>
                        <div className='container-swiper'>
                            <div className='slide-box'>
                                <div className='slider'>
                                    <div className='swiper'>
                                        <div className='swiper-content'>
                                            <div className='swiper-text'>
                                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione velit voluptates animi dolorem ad. Placeat.
                                            </div>
                                            <img src="" alt="" />
                                        </div>
                                    </div>
                                    <div className='swiper-content'>
                                        <div className='swiper-content-box'>
                                            <div className='text1'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Placeat, expedita?</div>
                                            <div className='swiper-img1'>
                                                <img src="" alt="" />
                                            </div>
                                            <div className='swiper-img2'>
                                                <img src="" alt="" />
                                            </div>
                                            <div className='swiper-img3'>
                                                <img src="" alt="" />
                                                <span className='swiper-span'>Tap to see more</span>
                                            </div>
                                            <div className='swiper-box-box'>
                                                <span></span>
                                                <span></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className='slide'>
                                        <div className='slide-container'>
                                            <div className='slide-box'>
                                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Cupiditate hic doloribus nemo similique sint aperiam?
                                            </div>
                                            <img src="" alt="" />
                                        </div>
                                    </div>
                                </div>
                                <div className='pagination'>
                                    <span className='pagination-bullet'></span>
                                    <span className='pagination-bullet'></span>
                                    <span className='pagination-bullet'></span>
                                </div>
                            </div>
                        </div>
                    </div> */}
                    <div className='pagination-item'>
                        <div>
                            <div>
                                <img src="" alt="" />
                                <img src="" alt="" />
                            </div>
                            <div></div>
                        </div>
                        <div className='page-content'>
                            <div className='page-content-box'>
                                <img src="" alt="" />
                            </div>
                            <div className='page-text'>
                                <h3>FIRST WEB3 CRYPTO</h3>
                                <h2>"Keeping track of projects in"
                                    <span>one place</span>
                                </h2>
                                <p>
                                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Est illo qui laudantium ut vel quo exercitationem omnis praesentium nostrum repellendus.
                                </p>
                                <div>
                                    <a href="">
                                        <button className='btn0 btn'>
                                            <div className='btn-0 btn-design'>
                                                ::after
                                            </div>
                                            <span></span>
                                            <div className='btn-0-text home-btn-text'>Start Creating</div>
                                            <div className='btn-0 btn-design'>
                                                ::after
                                            </div>
                                        </button>
                                    </a>
                                    <a href="">
                                        <button className='view-btn'>View Roadmap</button>
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='container-box'>
                        <div className='container-box-box'>
                            <h2>Find alpha before anyone else with us</h2>
                            <div className='container-content'>
                                <div className='container-div'>
                                    <img src="" alt="" />
                                    "Add a list of top Twitter accounts"
                                </div>
                                <div className='container-div2'>
                                    <img src="" alt="" />
                                    "Follow their new subscription"
                                </div>
                                <div className='container-div3'>
                                    <img src="" alt="" />
                                    "We'll show you matches on those subscription"
                                    <div className='container-div4'>
                                        <img src="" alt="" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className='select'>
                            <div className='selection'>
                                <div>
                                    <div>
                                        <img src="" alt="" />
                                        <div className='select-num'>
                                            <div>1</div>
                                            <div>2</div>
                                            <div>3</div>
                                            <div>4</div>
                                            <div>5+</div>
                                        </div>
                                    </div>
                                </div>
                                <img src="" alt="" />
                                <section className='section'>
                                    <div className='section-box'>
                                        <h3>Finding Alpha is easy</h3>
                                        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque minima repellat voluptates autem officia nesciunt!</p>
                                    </div>
                                    <div>
                                        <a href="">
                                            <button className='section-btn'>
                                                <div className='section-div'></div>
                                                <span></span>
                                                <div className='section-head'>Try now for free</div>
                                                <div className='section-div'></div>
                                            </button>
                                        </a>
                                        <a href="">
                                            <button className='section-btn2' >
                                                <div className='section-div2'></div>
                                                <span></span>
                                                <div className='section-head'>
                                                    <img src="" alt="" />
                                                    "Follow Twitter"
                                                </div>
                                                <div className='section-div2'></div>
                                            </button>
                                        </a>
                                    </div>
                                </section>
                            </div>
                        </div>
                    </div>
                    <div className='next-section'>
                        <div className='next-section-box'>
                            <div className='next-section-content'>
                                <div className='next-section-div'>
                                    "No span, only"
                                    <div className='next-section-word'>
                                        <div className='next-section-img'>
                                            <img src="" alt="" />
                                        </div>
                                        "Alpha"
                                    </div>
                                </div>
                                <div className='next-section-text'>
                                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente, animi!
                                </div>
                                <div className='next-section-text next-text'>
                                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente, animi!
                                </div>
                                <div className='next-section-imgtext'>
                                    <img src="" alt="" />
                                </div>
                                <div className='next-section-text next2'>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Et, repellendus!</div>
                                <div className='next-section-imgtext'>
                                    <img src="" alt="" />
                                </div>
                                <div className='next-section-text next-text'>
                                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente, animi!
                                </div>
                            </div>
                            <div className='next-img-section'>
                                <div className='next-img1'>
                                    <img src="" alt="" />
                                </div>
                                <div className='next-img2-box'>
                                    <div className='next-img2'>
                                        <img src="" alt="" />
                                    </div>
                                    <div className='next-img3'>
                                        <img src="" alt="" />
                                    </div>
                                </div>
                                <div className='next-img4'>
                                    <img src="" alt="" />
                                </div>
                            </div>
                            <div className='nextbox1'>
                                <div className='nextbox2'>
                                    <div className='nextbox3'></div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            {/* features page */}
            <div className='features'>
                <div className='features-container'>
                    <div className='feature-div'>
                        <div className='feature-div-img'></div>
                        <img src="" alt="" />
                        <img src="" alt="" />
                    </div>
                    <div className='features-box'>
                        <div className='feature-head-section'>
                            <h1 className='feature-heading'>Use all the benefits of NFD</h1>
                            <span className='feature-head-inro'>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Tenetur, molestiae.</span>
                        </div>
                        <div className='features-content-box'>
                            <div className='features-head-box'>
                                <div className='features-content'>
                                    <span>Activities</span>
                                    <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. Est, accusantium.</span>
                                </div>
                                <div className='feature-heading-div'>
                                    <span>Auto-Update feed</span>
                                    <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente maiores aut dolorem consequatur adipisci odio voluptates dolore praesentium nobis amet?</span>
                                </div>
                                <div className='feature-heading-div'>
                                    <span>Calender</span>
                                    <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. Id repellendus reprehenderit magni quis, alias esse sunt et nihil adipisci quas.</span>
                                </div>
                                <div className='feature-heading-div'>
                                    <span>Aggregator</span>
                                    <span>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Aspernatur unde, deserunt veniam deleniti soluta recusandae? Officiis sint corrupti in ex.</span>
                                </div>
                                <div className='feature-heading-div'>
                                    <span>Alpha Search</span>
                                    <span>Lorem ipsum dolor sit amet, consectetur adipisicing elit. A eos rerum nesciunt temporibus dolores nisi sit est asperiores minus assumenda.</span>
                                </div>
                            </div>
                            <div className='feature-content-img'>
                                <img src="https://www.nfd.gg/_next/image?url=%2Fbenefits%2FactivitiesBenefitOPT.webp&w=1920&q=100" alt="" />
                            </div>
                        </div>
                        <div className='feature-section2'>
                            <div className='feature-section-container'>
                                <div className='feature-section-box'>
                                    <div>
                                        <div className='feature-section-activity'>
                                            "0"
                                            "1"
                                        </div>
                                        <div className='feature-heading2'>Activities</div>
                                    </div>
                                    <img src="" alt="" />
                                </div>
                                <div className='feature-content2'>
                                    <span className='feature-text2'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatem quam eos placeat asperiores, animi aliquid.</span>
                                    <img src="" alt="" />
                                </div>
                            </div>
                            <div className='feature-section-container'>
                                <div className='feature-section-box'>
                                    <div>
                                        <div className='feature-section-activity'>
                                            "0"
                                            "2"
                                        </div>
                                        <div className='feature-heading2'>Auto-Update feed</div>
                                    </div>
                                    <img src="" alt="" />
                                </div>
                                <div className='feature-content2'>
                                    <span className='feature-text2'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatem quam eos placeat asperiores, animi aliquid.</span>
                                    <img src="" alt="" />
                                </div>
                            </div>
                            <div className='feature-section-container'>
                                <div className='feature-section-box'>
                                    <div>
                                        <div className='feature-section-activity'>
                                            "0"
                                            "3"
                                        </div>
                                        <div className='feature-heading2'>Calender</div>
                                    </div>
                                    <img src="" alt="" />
                                </div>
                                <div className='feature-content2'>
                                    <span className='feature-text2'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatem quam eos placeat asperiores, animi aliquid.</span>
                                    <img src="" alt="" />
                                </div>
                            </div>
                            <div className='feature-section-container'>
                                <div className='feature-section-box'>
                                    <div>
                                        <div className='feature-section-activity'>
                                            "0"
                                            "4"
                                        </div>
                                        <div className='feature-heading2'>Aggegator</div>
                                    </div>
                                    <img src="" alt="" />
                                </div>
                                <div className='feature-content2'>
                                    <span className='feature-text2'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatem quam eos placeat asperiores, animi aliquid.</span>
                                    <img src="" alt="" />
                                </div>
                            </div>
                            <div className='feature-section-container'>
                                <div className='feature-section-box'>
                                    <div>
                                        <div className='feature-section-activity'>
                                            "0"
                                            "5"
                                        </div>
                                        <div className='feature-heading2'>Alph search</div>
                                    </div>
                                    <img src="" alt="" />
                                </div>
                                <div className='feature-content2'>
                                    <span className='feature-text2'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatem quam eos placeat asperiores, animi aliquid.</span>
                                    <img src="" alt="" />
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='feature-slide-design'>
                        <div></div>
                        <img src="" alt="" />
                        <img src="" alt="" />
                    </div>
                </div>
            </div>

            {/* roadmap page */}
            <div className='roadmap-page'>
                <div className='roadmap-box'></div>
                <div className='raodmap-img1 raodmap-img'>
                    <img src="https://www.nfd.gg/_next/image?url=%2Fblurs%2Ffifth_block_one_elips_blur.webp&w=2048&q=75" alt="" />
                </div>
                <div className='raodmap-img1 raodmap-img'>
                    <img src="https://www.nfd.gg/_next/image?url=%2Fblurs%2Ffifth_block_two_elips_blur.webp&w=1080&q=75" alt="" />
                </div>
                <div className='raodmap-img1 raodmap-img'>
                    <img src="https://www.nfd.gg/_next/image?url=%2Fblurs%2Ffifth_block_two_elips_blur.webp&w=1080&q=75" alt="" />
                </div>
                <div className='roadmap-heading-box'>
                    <div className='roadmap-container-content'>
                        <div className='roadmap-content'>
                            <h1>NFD game changing crypto platform</h1>
                            <div className='roadmap-points'>
                                <div className='roadmap-head1'>
                                    <div></div>
                                    <span>Done</span>
                                </div>
                                <div className='roadmap-head2'>
                                    <div></div>
                                    <span>In Work</span>
                                </div>
                                <div className='roadmap-head3'>
                                    <div></div>
                                    <span>Upcoming</span>
                                </div>
                            </div>
                        </div>
                        <div className='roadmap-whole'>
                            <div className='whole-box'></div>
                            <div className='roadmap-whole-content'>
                                <div className='roadmap-line1'>
                                    <div className='roadmap-div1'></div>
                                    <span className='roadmap-span'></span>
                                    <div className='roadmap-div2'></div>
                                    <div className='roadmap-section1'>
                                        <div className='road-sec-content'>
                                            <div></div>
                                            <span>Development</span>
                                        </div>
                                        <div className='road-sec-content'>
                                            <div></div>
                                            <span>Activities and Aggregator</span>
                                        </div>
                                        <div className='road-sec-content'>
                                            <div></div>
                                            <span>Alpha test</span>
                                        </div>
                                        <div className='road-sec-content'>
                                            <div></div>
                                            <span>Alpha search tool</span>
                                        </div>
                                        <div className='road-sec-contentb'>
                                            <div></div>
                                            <span>Beta test</span>
                                        </div>
                                    </div>
                                </div>
                                <div className='roadmap-line1'>
                                    <div className='roadmap-div1'></div>
                                    <span className='roadmap-span'></span>
                                    <div className='roadmap-div2'></div>
                                    <div className='roadmap-section1'>
                                        <div className='road-sec-contentb'>
                                            <div></div>
                                            <span>ROI scanner</span>
                                        </div>
                                        <div className='road-sec-contentb'>
                                            <div></div>
                                            <span>Summary</span>
                                        </div>
                                        <div className='road-sec-contentb'>
                                            <div></div>
                                            <span>Twitter scanner tool</span>
                                        </div>
                                        <div className='road-sec-contentc'>
                                            <div></div>
                                            <span>Whale's analysis</span>
                                        </div>
                                        <div className='road-sec-contentb'>
                                            <div></div>
                                            <span>Blocks in Aggregator</span>
                                        </div>
                                    </div>
                                </div>
                                <div className='roadmap-line1'>
                                    <div className='roadmap-div1'></div>
                                    <span className='roadmap-span'></span>
                                    <div className='roadmap-div2'></div>
                                    <div className='roadmap-section1'>
                                        <div className='road-sec-contentb'>
                                            <div></div>
                                            <span>New sections in Activities</span>
                                        </div>
                                        <div className='road-sec-contentb'>
                                            <div></div>
                                            <span>Launching NFD DAO</span>
                                        </div>
                                        <div className='road-sec-contentb'>
                                            <div></div>
                                            <span>Expanding the Calender</span>
                                        </div>
                                        <div className='road-sec-contentb'>
                                            <div></div>
                                            <span>Token vesting tool</span>
                                        </div>
                                        <div className='road-sec-contentb'>
                                            <div></div>
                                            <span>NFD lab incubator</span>
                                        </div>
                                    </div>
                                </div>
                                <div className='roadmap-line1'>
                                    <div className='roadmap-div1'></div>
                                    <span className='roadmap-span'></span>
                                    <div className='roadmap-div2'></div>
                                    <div className='roadmap-section1'>
                                        <div className='road-sec-contentc'>
                                            <div></div>
                                            <span>Crypto artitrage scanner</span>
                                        </div>
                                        <div className='road-sec-contentc'>
                                            <div></div>
                                            <span>Alpha search AI developing based on data collected</span>
                                        </div>
                                        <div className='road-sec-contentc'>
                                            <div></div>
                                            <span>Adding any services based on NFD DAO votes</span>
                                        </div>
                                    </div>
                                </div>
                                <div className='roadmap-line1'>
                                    <div className='roadmap-div1'></div>
                                    <span className='roadmap-span'></span>
                                    <div className='roadmap-div2'></div>
                                    <div className='roadmap-section1'>
                                        <div className='road-sec-contentc'>
                                            <div></div>
                                            <span>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laudantium, nulla.</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        {/* <div>
                            <div>
                                <div>
                                    <div>
                                        <div>
                                            <div>
                                                <span></span>
                                                <div></div>
                                                <div>
                                                    <div>
                                                        <div></div>
                                                        <span>Development</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Activities and Aggregator</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Alpha test</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Alpha search tool</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Beta test</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <div>
                                            <div>
                                                <span></span>
                                                <div></div>
                                                <div>
                                                    <div>
                                                        <div></div>
                                                        <span>ROI scanner</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Aggregator summary</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Twitter Scanner tool</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Whale's analysis</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Blocks in Aggregator</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <div>
                                            <div>
                                                <span></span>
                                                <div></div>
                                                <div>
                                                    <div>
                                                        <div></div>
                                                        <span>New section on Activities</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Launching NFD DAO </span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Expanding the calender</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Token vesting tool</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>NFD labs incubator</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <div>
                                            <div>
                                                <span></span>
                                                <div></div>
                                                <div>
                                                    <div>
                                                        <div></div>
                                                        <span>Crypto arbitrage scanner</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Officia, suscipit!</span>
                                                    </div>
                                                    <div>
                                                        <div></div>
                                                        <span>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Quaerat, possimus.</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <div>
                                            <div>
                                                <span></span>
                                                <div></div>
                                                <div>
                                                    <div>
                                                        <div></div>
                                                        <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla labore laudantium neque recusandae porro. Pariatur?</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <span></span>
                                    <span></span>
                                    <span></span>
                                    <span></span>
                                </div>
                            </div>
                        </div> */}
                    </div>
                </div>
            </div>

            {/* section  */}
            <div>
                <div>
                    <div>
                        <img src="" alt="" />
                    </div>
                    <div>
                        <img src="" alt="" />
                    </div>
                </div>
                <div>
                    <div>
                        <div></div>
                        <img src="" alt="" />
                        <img src="" alt="" />
                    </div>
                    <div>
                        <div>
                            <h1>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, molestiae?</h1>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Architecto explicabo perspiciatis at tempore expedita quas itaque repellat commodi soluta tempora?</p>
                            <button>
                                <div></div>
                                <span></span>
                                <div>Fill out a form</div>
                                <div></div>
                            </button>
                        </div>
                        <div>
                            <div>
                                <div>
                                    <div>
                                        <span>More info</span>
                                    </div>
                                    <div>
                                        <div>
                                            <div>
                                                <div>
                                                    <img src="" alt="" />
                                                </div>
                                            </div>
                                            <div>
                                                <span>William</span>
                                                <span>CTO</span>
                                            </div>
                                        </div>
                                        <div>
                                            <span>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Sapiente, distinctio!</span>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <div>
                                        <div></div>
                                    </div>
                                    <div>
                                        <img src="" alt="" />
                                    </div>
                                </div>
                                <div>
                                    <h4>William</h4>
                                    <span>CTO</span>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>
                                        <span>More info</span>
                                    </div>
                                    <div>
                                        <div>
                                            <div>
                                                <div>
                                                    <img src="" alt="" />
                                                </div>
                                            </div>
                                            <div>
                                                <span>Leo</span>
                                                <span>CEO</span>
                                            </div>
                                        </div>
                                        <div>
                                            <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. Numquam nobis pariatur ipsa ipsum. Dolores, soluta.</span>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <div>
                                        <div></div>
                                    </div>
                                    <div>
                                        <img src="" alt="" />
                                    </div>
                                </div>
                                <div>
                                    <h4>Leo</h4>
                                    <span>CEO</span>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>
                                        <span>More info</span>
                                    </div>
                                    <div>
                                        <div>
                                            <div>
                                                <div>
                                                    <img src="" alt="" />
                                                </div>
                                            </div>
                                            <div>
                                                <span>Vadim</span>
                                                <span>CCO</span>
                                            </div>
                                        </div>
                                        <div>
                                            <span>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Amet ipsum odit optio reiciendis consequuntur iure.</span>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <div>
                                        <div></div>
                                    </div>
                                    <div>
                                        <img src="" alt="" />
                                    </div>
                                </div>
                                <div>
                                    <h4>Vadim</h4>
                                    <span>CCO</span>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>
                                        <span>More info</span>
                                    </div>
                                    <div>
                                        <div>
                                            <div>
                                                <div>
                                                    <img src="" alt="" />
                                                </div>
                                            </div>
                                            <div>
                                                <span>Vitale</span>
                                                <span>Team Lead fullstack</span>
                                            </div>
                                        </div>
                                        <div>
                                            <span>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Corrupti consectetur, nisi obcaecati quod aliquam facilis!</span>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <div>
                                        <div></div>
                                    </div>
                                    <div>
                                        <img src="" alt="" />
                                    </div>
                                </div>
                                <div>
                                    <h4>Vitale</h4>
                                    <span>Team Lead fullstack</span>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>
                                        <span>More info</span>
                                    </div>
                                    <div>
                                        <div>
                                            <div>
                                                <div>
                                                    <img src="" alt="" />
                                                </div>
                                            </div>
                                            <div>
                                                <span>Nikita</span>
                                                <span>Frontend Developer</span>
                                            </div>
                                        </div>
                                        <div>
                                            <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. Qui fugiat nisi, nam rerum ex dolor?</span>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <div>
                                        <div></div>
                                    </div>
                                    <div>
                                        <img src="" alt="" />
                                    </div>
                                </div>
                                <div>
                                    <h4>Nikita</h4>
                                    <span>Frontend Developer</span>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>
                                        <span>More info</span>
                                    </div>
                                    <div>
                                        <div>
                                            <div>
                                                <div>
                                                    <img src="" alt="" />
                                                </div>
                                                <div>
                                                    <span>Den</span>
                                                    <span>Backend developer</span>
                                                </div>
                                            </div>
                                            <div>
                                                <span>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Error, dolor.</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <div>
                                            <div></div>
                                        </div>
                                        <div>
                                            <img src="" alt="" />
                                        </div>
                                    </div>
                                    <div>
                                        <h4>Den</h4>
                                        <span>Backend developer</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div>
                            <div></div>
                            <img src="" alt="" />
                            <img src="" alt="" />
                        </div>
                    </div>
                </div>
            </div>


            {/* contact page */}
            <div>
                <div>
                    <div>
                        <img src="" alt="" />
                    </div>
                    <div>
                        <img src="" alt="" />
                    </div>
                    <div>
                        <img src="" alt="" />
                    </div>
                    <div>
                        <img src="" alt="" />
                    </div>
                    <div>
                        <img src="" alt="" />
                    </div>
                    <div>
                        <h3>Join our community</h3>
                        <div>
                            <a href="">
                                <button>
                                    <div></div>
                                    <span></span>
                                    <div>
                                        <img src="" alt="" />
                                        Join Discord
                                    </div>
                                    <div></div>
                                </button>
                            </a>
                            <a href="">
                                <button>
                                    <div></div>
                                    <span></span>
                                    <div>
                                        <img src="" alt="" />
                                        Follow twitter
                                    </div>
                                    <div></div>
                                </button>
                            </a>
                        </div>
                        <a href="">
                            <span>Read our whitepaper</span>
                        </a>
                    </div>
                </div>
            </div>
            <div>
                <div>
                    <div>
                        <div>
                            <div>
                                <img src="" alt="" />
                                <span>c 2023 NFD, INC. ALL RIGHTS RESERVED.</span>
                                <div>
                                    <a href="">Privacy</a>
                                    <a href="">Term of use</a>
                                </div>
                            </div>
                            <div>
                                <a href="">contact2nfd.gg</a>
                                <a href="">
                                    <button>
                                        <div></div>
                                        <span></span>
                                        <div>Try app</div>
                                        <div></div>
                                    </button>
                                </a>
                            </div>
                        </div>
                        <div>
                            <a href="">Privacy</a>
                            <a href="">Term of use</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Navbar
